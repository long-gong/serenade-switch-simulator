...
class Serenade_Conservative_for_Proof : virtual public Scheduler {
public:
  ...
  Serenade_Conservative_for_Proof(int p, std::vector<int> &v)
      : Scheduler(p), r_matching(p, 0), s_output_matched(p, 0),
        merged_matching(p, 0), Ouroboros(v), internal_counter(0){};
  virtual void run(lws_param_t &g_params, lws_status_t &g_status,
                   lws_inst_t &g_instruments);
  virtual void reset() { internal_counter = 0; }
  void arrival_matching_greedy(lws_param_t &g_params, lws_status_t &g_status);
};

void Serenade_Conservative_for_Proof::arrival_matching_greedy(
    lws_param_t &g_params, lws_status_t &g_status) {
...
void Serenade_Conservative_for_Proof::run(lws_param_t &g_params,
                                          lws_status_t &g_status,
                                          lws_inst_t &g_instruments) {
  assert(g_params.N == port_num);

  if (internal_counter == 0) { /* initial */
    if (!UT::is_matching(g_status.S))
  ...
  UT::C_merge(g_status.S, r_matching, merged_matching, g_status.Q, Ouroboros,
              g_instruments);
