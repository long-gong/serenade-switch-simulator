...
public:
    ...
    E_Serenade_(int p, std::vector<int>& v):\
    Scheduler(p),  r_matching(p, 0), s_output_matched(p, 0),\
    merged_matching(p, 0), Ouroboros(v), internal_counter(0), Q_bound(std::numeric_limits<int>::max()), frequency(100) {};

    E_Serenade_(int p, std::vector<int>& v, int qb, double freq):\
    Scheduler(p),  r_matching(p, 0), s_output_matched(p, 0),\
    merged_matching(p, 0), Ouroboros(v), internal_counter(0), Q_bound(qb),frequency(freq) {};
...
void E_Serenade_::run(lws_param_t &g_params, lws_status_t &g_status, lws_inst_t &g_instruments){
    ...
    if (distribution(generator) <= 1.0 / frequency)
        UT::merge(g_status.S, r_matching, merged_matching, g_status.Q);
    else
        UT::O_merge(g_status.S, r_matching, merged_matching, g_status.Q,
